return {
  ...
  highlight DapStoppedLine guibg=#3c3836 guifg=#ffffff
			...
			-- Custom DAP configurations for C/C++ with dynamic arguments
			dap.configurations.cpp = {
				{
					name = "Launch",
					type = "codelldb", -- Use "codelldb" or "lldb"
					request = "launch",
					program = function()
					...
					-- Dynamic args input
					args = function()
						local input = vim.fn.input("Arguments: ") -- Prompt for arguments
						return vim.fn.split(input, " ") -- Split the input into arguments
					end,
				},
			}

			-- Reuse for C programs if needed
